...
public class AssetsOld {
    ...
    public static void load(GLGameGPGS game) {
        ...
        mainMenu = new TextureRegion(items, 0, 352, 352, 160);
        pauseMenu = new TextureRegion(items, 224, 128, 192, 96);
        ready = new TextureRegion(items, 320, 224, 192, 32);
        gameOver = new TextureRegion(items, 352, 256, 160, 96);
        highScoresRegion = new TextureRegion(Assets.items, 32, 400, 289, 26);
        logo = new TextureRegion(items, 0, 224, 256, 32);
        soundOff = new TextureRegion(items, 0, 0, 64, 64);
        soundOn = new TextureRegion(items, 64, 0, 64, 64);
        arrow = new TextureRegion(items, 0, 64, 64, 64);
        pause = new TextureRegion(items, 64, 64, 64, 64);
        gplusSignIn = new TextureRegion(items, 0, 256, 128, 64);
        signOut = new TextureRegion(items, 0, 320, 128, 32);
        
        
        spring = new TextureRegion(items, 128, 0, 32, 32);
        castle = new TextureRegion(items, 128, 64, 64, 64);
        projectile = new TextureRegion(items, 192, 32, 32, 32);
