...
import se.stjerndal.androidgames.framework.glgpgs.FPSCounter;
...
import se.stjerndal.androidgames.gnagetjump.World.WorldListener;

public class GameScreen extends GLScreenGPGS {
    ...
    public GameScreen(Game game) {
        ...
        shootBounds = new Rectangle(0, 0, 320, 480 - 64);
        lastScore = 0;
        scoreString = "score: 0";
    ...
    private void updateRunning(float deltaTime) {
        ...
        for(int i = 0; i < len; i++) {
            ...
            if(event.type == TouchEvent.TOUCH_DOWN) {
            	if (OverlapTester.pointInRectangle(shootBounds, touchPoint)) {
            		world.shoot();
					Assets.playSound(Assets.shootSound);
					//TODO - Make the sound be played via WorldListener instead
					break;
				}   
            }
        ...
        if(world.state == World.WORLD_STATE_GAME_OVER) {
            ...
            if(glGame.isSignedIn()) {
				glGame.submitNormalHighscore(lastScore);
				glGame.unlockAchievement(glGame
						.getString(R.string.achievement_solen_stiger));
				glGame.incrementAchievement(glGame
						.getString(R.string.achievement_third_times_the_charm));
				glGame.incrementAchievement(glGame
						.getString(R.string.achievement_the_first_five));
				glGame.incrementAchievement(glGame
						.getString(R.string.achievement_centurion));
            }
    ...
    private void updateGameOver() {
        ...
        int len = touchEvents.size();
        for(int i = 0; i < len; i++) {
        	//TODO - Bug: spamming shoots renders indexOoBExc, i = 1 len = 0
            TouchEvent event = touchEvents.get(i);
            if(event.type != TouchEvent.TOUCH_UP)
                continue;
            game.setScreen(new MainMenuScreen(game));
